package flare.physics
{
	 ...
	 * </ul></p>
	 */
	public class NBodyForce2 implements IForce
	{
		...
		/**
		 * Creates a new NBodyForce2 with given parameters.
		 ...
		 */
		public function NBodyForce2(g:Number=-1, max:Number=200, min:Number=2,
		...
		public function apply2(sim:Simulation2):void
		{
        	...
        	for (var i:uint = 0; i<sim.particles.length; ++i) {
        		if(sim.particles[i].fixed == false)
		...
		private function forces(p:Particle2, n:QuadTreeNode,
		...
		private function insert(p:Particle2, n:QuadTreeNode,
			x1:Number, y1:Number, x2:Number, y2:Number):void
		{
		...
		private function insertHelper(p:Particle2, n:QuadTreeNode, 
			x1:Number, y1:Number, x2:Number, y2:Number):void
    	{
    		// determine split
			var sx:Number = (x1+x2)/2;
		...
		private function bounds(sim:Simulation2):void
		{
			var p:Particle2, dx:Number, dy:Number;
			_x1 = _y1 = Number.MAX_VALUE;
			_x2 = _y2 = Number.MIN_VALUE;

			// get bounding box
			for (var i:uint = 0; i<sim.particles.length; ++i) {
				p = sim.particles[i] as Particle2;
				if (p.x < _x1) _x1 = p.x;
				if (p.y < _y1) _y1 = p.y;
				if (p.x > _x2) _x2 = p.x;
				if (p.y > _y2) _y2 = p.y;
		...
		private function isSameLocation(p1:Particle2, p2:Particle2):Boolean {
        	return (Math.abs(p1.x - p2.x) < _eps && 
        			Math.abs(p1.y - p2.y) < _eps);
    	}
		
	} // end of class NBodyForce2
}
