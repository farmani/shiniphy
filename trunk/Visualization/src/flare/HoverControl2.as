package flare
{
	 ...
	 */
	public class HoverControl2 extends Control
	{
		 ...
		 */
		public function HoverControl2(filter:*=null, movePolicy:int=DONT_MOVE,
		...
		private function onMouseOver(evt:MouseEvent):void
		{
			...
			if(n is MovieSprite){ (n as MovieSprite).isHover = true;(n as MovieSprite).redraw();}
		...
		{
			var n:MovieSprite = evt.target as MovieSprite;
			if(n != null){ n.isHover = false;n.redraw();}
			if (_cur == null) return;
			if (hasEventListener(SelectionEvent.DESELECT)) {
				dispatchEvent(
					new SelectionEvent(SelectionEvent.DESELECT, _cur, evt));
			}
			if (_movePolicy == MOVE_AND_RETURN && _cur.parent != null) {
				_cur.parent.setChildIndex(_cur, _idx);
			}
			_cur = null;
		}
		
	} // end of class HoverControl2
}